...
rule lca_mgkit:
    ...
    shell:
        """
        taxon-utils lca -b {params.bitscore_threshold} -s -p -n {output.no_lca} -t /mnt/db/taxdb/taxonomy.pickle {input} {output.taxtab} > {log} 2>&1
        if [[ ! -e {output.no_lca} ]]; then
        touch {output.no_lca}
        fi
        python bin/average_logevalue_no_lca.py {output.taxtab} {output.no_lca} {input} {output.taxtab} >> {log} 2>&1 
        """
rule lca_output:
    input:
        taxtab="data/taxonomic_classification/{sample}.taxtab",
    ...
    output:
        "data/taxonomic_classification/{sample}.taxMagtab"
    conda:
        "../envs/mgkit_lca.yaml"
    benchmark:
        "logs/benchmark/lca_output_{sample}.txt"
    threads: 1
    log:
        "logs/lca_output_{sample}.log"
    shell:  
        """
        python bin/krona_magnitudes.py {input.taxtab} {input.stats} {output} >> {log} 2>&1
        """