...
def main():
    ...
    pattern = re.compile(r".*(_|\.)R?1([_.].*$|$)") # A regex meaning: get anything up to "_R?1" and then either "_R?1[_.].*$" or "_R?1$".
    pre_qc = [ col for col in read_nrs.Sample if bool(re.search(pattern, col)) ]
    read_nrs = read_nrs[read_nrs.Sample.isin(pre_qc)]
    read_nrs.Sample = read_nrs.Sample.str.replace(r'(_|\.)R?1([_.].*$|$)', '') #remove the "_R1" or "_1" suffix and anything that may be after it. N.B. a simple `.*`` doesn't work because otherwise it will greedily remove everything after the first occurrence of "_R1" e.g. "Sample_bla_bla_R138_ACGGT_R1" becomes "Sample_bla_bla"
    ...
    lowq_nrs.Sample = lowq_nrs.Sample.str.replace(r'(_|\.)R?1([_.].*$|$)', '') #remove the "_R1" or "_1" suffix and anything that may be after it. N.B. a simple `.*`` doesn't work because otherwise it will greedily remove everything after the first occurrence of "_R1" e.g. "Sample_bla_bla_R138_ACGGT_R1" becomes "Sample_bla_bla"
