...
def MakeBedFrame(primers, reference):
    ...
    for record in SeqIO.parse(primers, "fasta"):
        ...
        if any(orient in record.id for orient in right) is True:
            ref, start, end, id = search_primers(record.seq.reverse_complement(), reference, record.id)

        bed = bed.append(pd.DataFrame({"chrom": ref, "start": start, "stop": end, "name": id}, index=[0]), ignore_index=True)
...
def IndexReads(fastqfile):
    ...
    for record in SeqIO.parse(fastq_io, "fastq"):
        ...
        ReadDict[i] = {"Readname": str(record.id), "Sequence": str(record.seq), 'Qualities': str(RecordQualities)}
        i = i + 1
    ...
    for hit in Aln.map(seq):
        ...
        if is_reverse is False:
            ...
            while (readstart in slimlist) is True:
                ...
                readseq, readqual, readstart = slice_forward_left(readstart, readseq, readqual)

            while (readend in widelist) is True:

                readseq, readqual, readend = slice_forward_right(readend, readseq, readqual)
        ...
        if is_reverse is True:
            ...
            while (readstart in revlist) is True:
                ...
                readseq, readqual, readstart = slice_reverse_left(readstart, readseq, readqual)

            while (readend in widelist) is True:

                readseq, readqual, readend = slice_reverse_right(readend, readseq, readqual)
...
if __name__ == "__main__":
    ...
    ReadFrame["ProcessedReads"] = ReadFrame.apply(Cut_FastQ, args=(bed, reference, slimlist, revlist, widelist), axis=1)
