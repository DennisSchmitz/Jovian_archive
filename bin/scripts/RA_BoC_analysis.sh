...
###            sample_name_BoC.vcf sample_name_BoC_pct.tsv sample_name_BoC_int.tsv                                ###
###     Output:                                                                                                   ###
...
# Generate a vcf
samtools mpileup -f ${INPUT_REF}  ${INPUT_BAM} -o ${OUTPUT_VCF}
...
BoC_at_threshold_1=$(cat ${OUTPUT_VCF} | gawk -v X="1" '$4>=X' | wc -l)
BoC_at_threshold_1_pct=$(echo "scale=4 ; ${OUTPUT_VCF} / ${ref_total_size} " | bc )
BoC_at_threshold_5=$(cat ${OUTPUT_VCF} | gawk -v X="5" '$4>=X' | wc -l)
BoC_at_threshold_5_pct=$(echo "scale=4 ; ${BoC_at_threshold_5} / ${ref_total_size} " | bc )
BoC_at_threshold_10=$(cat ${OUTPUT_VCF} | gawk -v X="10" '$4>=X' | wc -l)
BoC_at_threshold_10_pct=$(echo "scale=4 ; ${BoC_at_threshold_10} / ${ref_total_size} " | bc )
BoC_at_threshold_30=$(cat ${OUTPUT_VCF} | gawk -v X="30" '$4>=X' | wc -l)
BoC_at_threshold_30_pct=$(echo "scale=4 ; ${BoC_at_threshold_30} / ${ref_total_size} " | bc )
BoC_at_threshold_100=$(cat ${OUTPUT_VCF} | gawk -v X="100" '$4>=X' | wc -l)
