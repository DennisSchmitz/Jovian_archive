#!/usr/bin/env python3
"""Parser for the XML output of the norovirus (HAV) typing tool (TT).

Usage:
  typingtool_HAV_XML_to_csv_parser.py <sample_name> <input_HAV_XML> <output_HAV_csv>

<sample_name> is how you want to call this sample (in plain text).
<input_HAV_XML> is the input XML file that you got from the HAV TT.
<output_HAV_csv> is the output csv file.

Example:
  typingtool_HAV_XML_to_csv_parser.py [sample_name] \
    data/virus_typing_tables/[sample_name]_HAV.xml \
    data/virus_typing_tables/[sample_name]_HAV.csv
"""
