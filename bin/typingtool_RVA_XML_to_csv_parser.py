#!/usr/bin/env python3
"""Parser for the XML output of the rotavirus A (RVA) typing tool (TT).

Usage:
  typingtool_RVA_XML_to_csv_parser.py <sample_name> <input_RVA_XML> <output_RVA_csv>

<sample_name> is how you want to call this sample (in plain text).
<input_RVA_XML> is the input XML file that you got from the RVA TT.
<output_RVA_csv> is the output csv file.

Example:
  typingtool_RVA_XML_to_csv_parser.py [sample_name] \
    data/virus_typing_tables/[sample_name]_RVA.xml \
    data/virus_typing_tables/[sample_name]_RVA.csv
"""
...
output_fields = [ 'Sample_name', 'Query_name', 'start', 'length', 'end', 'blast_concluded-name',
                 ...
                 'cluster_type', 'cluster_type_support', 'nucleotides' ]
        ...
        if item.tag in input_fields:
            if item.tag=='nucleotides':
                ...
                if tmp_conclusion_type=="simple" and tmp_conclusion_id=="type":
                    ...
                    inner_dict['cluster_type_support'] = item.find('.//support').text
            else:
