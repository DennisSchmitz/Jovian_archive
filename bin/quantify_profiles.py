...
def main():
    ...
    read_nrs.Sample = read_nrs.Sample.str.replace(r'_R?1([_.].*$|$)', '') #remove the "_R1" or "_1" suffix and anything that may be after it. N.B. a simple `.*`` doesn't work because otherwise it will greedily remove everything after the first occurance of "_R1" e.g. "Sample_bla_bla_R138_ACGGT_R1" becomes "Sample_bla_bla"
    ...
    lowq_nrs.Sample = lowq_nrs.Sample.str.replace(r'_R?1[_.\s].*', '') #remove the "_R1" or "_1" suffix and anything that may be after it
